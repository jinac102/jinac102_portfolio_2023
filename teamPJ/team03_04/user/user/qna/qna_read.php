...
<main>
  ...
  <?php 
    ...
    // print_r($qna_current_idx);
    // print_r($rsc[$qna_current_idx+1]);
    $prev_qidx = $rsc[$qna_current_idx+1];
    $next_qidx = $rsc[$qna_current_idx-1];
    // $qidx_arr_no = count($rsc);
    // $qna_current_arr = $rsc[$qna_current_idx];
    // print_r($next_qidx);
    // print_r($prev_qidx);
  ...
  <div class="user_question_info qna_container">
    <div class="user_info">
      ...
      <div class="user d-flex align-items-center justify-content-between">
        <div class="d-flex align-items-center suit_rg_s">
          <img src="../img/pabcon.png" alt="profile" />
    ...
    <?php if($r->reply_st == 1){ ?><!-- if(답변완료라면, 답변의 상태가 1이라면) -->
      <div class="admin_reply_section qna_container">
        <div class="admin_info d-flex align-items-center">
          ...
          <?php }else{ ?>
              <img src="<?php echo $r->user_file?>" alt="" class="adminphoto"/>
          <?php } ?>
          <div>
            <p class="admin_id suit_rg_s"><?php echo $rs->userid;?></p>
