...
<?php
  ...
  $search_keyword .= $category.'={'.$keyword.'}';//검색타입 조건 추가하여 조회
  $order = " order by reply_st desc, qidx desc, qna_recom asc";//마지막에 등록한걸 먼저 보여줌
  $limit= " limit $start_num,7";
...
<div class="banner">
  ...
  <div class="lists qna_container">
    ...
    <?php
        // $sql = "SELECT * FROM lms_qna ORDER BY reply_st desc, qidx desc, qna_recom asc limit $start_num,$list";
        ...
        <?php
        // $sql = "SELECT * FROM lms_qna ORDER BY reply_st desc, qidx desc, qna_recom asc limit $start_num,$list";
        // $sql = "SELECT qna_recom FROM lms_qna ORDER BY qna_recom desc limit 0,3";
